...
namespace PetoonsStudio.PSEngine.Utils
{
    ...
    {
        public static MMConsole m_Console;
        ...
        public static RaycastHit2D CircleCast(Vector2 rayOriginPoint, Vector2 rayDirection, float circleRadius, float rayDistance, LayerMask mask, Color color, bool drawGizmo = false)
        {
            if (drawGizmo)
            {
                Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color);
            }
            return Physics2D.CircleCast(rayOriginPoint, circleRadius, rayDirection, rayDistance, mask);
        }

        public static RaycastHit2D[] CircleCastAll(Vector2 rayOriginPoint, Vector2 rayDirection, float circleRadius, float rayDistance, LayerMask mask, Color color, bool drawGizmo = false)
        {
            if (drawGizmo)
            {
                Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color);
            }
            return Physics2D.CircleCastAll(rayOriginPoint, circleRadius, rayDirection, rayDistance, mask);
        }
        ...
        public static void DebugOnScreen(string label, object value, int fontSize = 10)
        {
            ...
            m_Console.AddMessage("<b>" + label + "</b> : " + value);
        }
        ...
        public static void InstantiateOnScreenConsole(int fontSize = 10)
        {
            if (m_Console == null)
            {
                ...
                m_Console = newGameObject.AddComponent<MMConsole>();
                m_Console.SetFontSize(fontSize);
            }
        }
        ...
        /// <param name="color">Color.</param>
        public static void DrawGizmoArrow(Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 3.00f, float arrowHeadAngle = 25.0f)
        {
            Gizmos.color = color;
        ...
        /// <param name="direction">Direction.</param>
        /// <param name="color">Color.</param>
        public static void DebugDrawArrow(Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 0.20f, float arrowHeadAngle = 35.0f)
        {
        ...
        public static void DrawGizmoCircle(Vector3 center, Vector3 normal,float radius, Color color)
        {
#if UNITY_EDITOR
            Handles.color = color;
        ...
        public static void DrawGizmoPlane(Vector3 position, Vector3 normal, float scale, Color color, float alpha = 1f)
        {
            ...
            color.a = alpha;
            Gizmos.color = color;
            Gizmos.DrawCube(Vector3.zero, new Vector3(scale, scale, 0.0001f));
            Gizmos.matrix = Matrix4x4.identity;
        }

        /// <summary>
        /// Debug logs with custom color
        /// </summary>
        /// <param name="message"></param>
        public static void GameLogFormat(string message)
        {
