...
namespace PetoonsStudio.PSEngine.Utils
{
    ...
    /// </summary>
    [ExecuteInEditMode]
    public static class PSEventManager
    {
        private static Dictionary<Type, List<PSEventListenerBase>> m_SubscribersList;

        static PSEventManager()
        {
            m_SubscribersList = new Dictionary<Type, List<PSEventListenerBase>>();
        }
        ...
        /// <typeparam name="PSEvent">The event type.</typeparam>
        public static void AddListener<PSEvent>(PSEventListener<PSEvent> listener) where PSEvent : struct
        {
            Type eventType = typeof(PSEvent);

            if (!m_SubscribersList.ContainsKey(eventType))
                m_SubscribersList[eventType] = new List<PSEventListenerBase>();
        ...
        /// <param name="listener">listener.</param>
        /// <typeparam name="PSEvent">The event type.</typeparam>
        public static void RemoveListener<PSEvent>(PSEventListener<PSEvent> listener) where PSEvent : struct
        {
            Type eventType = typeof(PSEvent);
            ...
            List<PSEventListenerBase> subscriberList = m_SubscribersList[eventType];
            bool listenerFound;
            listenerFound = false;
        ...
        /// <typeparam name="PSEvent">The 1st type parameter.</typeparam>
        public static void TriggerEvent<PSEvent>(PSEvent newEvent) where PSEvent : struct
        {
            List<PSEventListenerBase> list;
            if (!m_SubscribersList.TryGetValue(typeof(PSEvent), out list))
            ...
            {
                (listener as PSEventListener<PSEvent>).OnPSEvent(newEvent);
            }
        }
        ...
        private static bool SubscriptionExists(Type type, PSEventListenerBase receiver)
        {
            List<PSEventListenerBase> receivers;
    ...
    public static class EventRegister
    {
        ...
        public static void PSEventStartListening<EventType>(this PSEventListener<EventType> caller) where EventType : struct
        {
            PSEventManager.AddListener<EventType>(caller);
        }

        public static void PSEventStopListening<EventType>(this PSEventListener<EventType> caller) where EventType : struct
        {
            PSEventManager.RemoveListener<EventType>(caller);
        }
    }
    ...
    /// </summary>
    public interface PSEventListenerBase { };

    /// <summary>
    /// A public interface you'll need to implement for each type of event you want to listen to.
    /// </summary>
    public interface PSEventListener<T> : PSEventListenerBase
    {
        void OnPSEvent(T eventType);
    }
}