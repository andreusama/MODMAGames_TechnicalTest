...
namespace MoreMountains.Tools
{
		...
		MMEventListener<MMSoundManagerTrackFadeEvent>
	{
		...
		public enum MMSoundManagerTracks { Sfx, Music, UI, Master, Voices, Other }

		[MMReadOnly] public MMSoundManagerSettingsSO settingsSo;
        ...
        public Dictionary<MMSoundManagerSound, bool> _cachedSFXState = new Dictionary<MMSoundManagerSound, bool>();
        public Dictionary<MMSoundManagerSound, bool> _cachedMusicState = new Dictionary<MMSoundManagerSound, bool>();
        public Dictionary<MMSoundManagerSound, bool> _cachedUIState = new Dictionary<MMSoundManagerSound, bool>();
        public Dictionary<MMSoundManagerSound, bool> _cachedMasterState = new Dictionary<MMSoundManagerSound, bool>();
        public Dictionary<MMSoundManagerSound, bool> _cachedVoicesState = new Dictionary<MMSoundManagerSound, bool>();
        public Dictionary<MMSoundManagerSound, bool> _cachedOtherState = new Dictionary<MMSoundManagerSound, bool>();
		...
		)
		{
			...
			if (!audioSource)
			{
				...
				if ((audioSource) && (!loop))
				{
					...
					StartCoroutine(_pool.AutoDisableAudioSource(audioClip.length / Mathf.Abs(pitch), audioSource, audioClip, doNotAutoRecycleIfNotDonePlaying, playbackTime, playbackDuration, scaledTime));
				}
			}
			...
			if (settingsSo != null)
			{
				...
				switch (mmSoundManagerTrack)
				{
					...
					case MMSoundManagerTracks.UI:
						...
						break;
					case MMSoundManagerTracks.Voices:
						audioSource.outputAudioMixerGroup = settingsSo.VoicesAudioMixerGroup;
						break;
				}
			}
		...
		public virtual float GetTrackVolume(MMSoundManagerTracks track, bool mutedVolume)
		{
			switch (track)
			{
                ...
                case MMSoundManagerTracks.Voices:
                    if (mutedVolume)
                    {
                        return settingsSo.Settings.MutedVoicesVolume;
                    }
                    else
                    {
                        return settingsSo.Settings.VoicesVolume;
                    }
            }
        ...
        /// <summary>
        /// Unpauses all sounds on a track
        /// </summary>
        /// <param name="track"></param>
        public virtual void UnPauseTrack(MMSoundManagerTracks track)
        {
            foreach (MMSoundManagerSound sound in _sounds)
            {
                if (sound.Track == track)
        ...
        /// <summary>
        /// Mutes the voices track, QoL method ready to bind to a UnityEvent
        /// </summary>
        public virtual void MuteVoices() { MuteTrack(MMSoundManagerTracks.Voices); }

        /// <summary>
        /// Unmutes the master track, QoL method ready to bind to a UnityEvent
        /// </summary>
        public virtual void UnmuteVoices() { UnmuteTrack(MMSoundManagerTracks.Voices); }
        ...
        /// <summary>
        /// Sets the volume of the Voices track to the specified value, QoL method, ready to bind to a UnityEvent
        /// </summary>
        public virtual void SetVolumeVoices(float newVolume) { SetTrackVolume(MMSoundManagerTracks.Voices, newVolume); }
        ...
        public virtual bool IsMuted(MMSoundManagerTracks track)
		{
			switch (track)
			{
                ...
                case MMSoundManagerTracks.Voices:
                    return !settingsSo.Settings.VoicesOn;
            }
			return false;
		}
		...
		protected virtual void ControlTrack(MMSoundManagerTracks track, ControlTrackModes trackMode, float volume = 0.5f)
		{
			...
			switch (track)
			{
				...
				case MMSoundManagerTracks.UI:
					...
					break;
                case MMSoundManagerTracks.Voices:
                    target = settingsSo.Settings.VoicesVolumeParameter;
                    if (trackMode == ControlTrackModes.Mute) { settingsSo.TargetAudioMixer.GetFloat(target, out settingsSo.Settings.MutedVoicesVolume); settingsSo.Settings.VoicesOn = false; }
                    else if (trackMode == ControlTrackModes.Unmute) { savedVolume = settingsSo.Settings.MutedVoicesVolume; settingsSo.Settings.VoicesOn = true; }
                    break;
            }
        ...
        public virtual void CacheTrackState(MMSoundManagerTracks track)
        {
            foreach (MMSoundManagerSound sound in _sounds)
            {
                if (sound.Track == track)
                {
                    ...
                    {
                        case MMSoundManagerTracks.Sfx:
                            if (!_cachedSFXState.ContainsKey(sound))
                                _cachedSFXState.Add(sound, sound.Source.isPlaying);
                            break;
                        case MMSoundManagerTracks.Music:
                            if (!_cachedMusicState.ContainsKey(sound))
                                _cachedMusicState.Add(sound, sound.Source.isPlaying);
                            break;
                        case MMSoundManagerTracks.UI:
                            if (!_cachedUIState.ContainsKey(sound))
                                _cachedUIState.Add(sound, sound.Source.isPlaying);
                            break;
                        case MMSoundManagerTracks.Master:
                            if (!_cachedMasterState.ContainsKey(sound))
                                _cachedMasterState.Add(sound, sound.Source.isPlaying);
                            break;
                        case MMSoundManagerTracks.Voices:
                            if (!_cachedVoicesState.ContainsKey(sound))
                                _cachedVoicesState.Add(sound, sound.Source.isPlaying);
                            break;
                        case MMSoundManagerTracks.Other:
                            if (!_cachedOtherState.ContainsKey(sound))
                                _cachedOtherState.Add(sound, sound.Source.isPlaying);
                            break;
                    }
                }
            }
        ...
        public virtual void RestorePreviousTrackState(MMSoundManagerTracks track)
        {
            switch (track)
            {
                case MMSoundManagerTracks.Sfx:
                    foreach (var sound in _cachedSFXState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
                            sound.Key.Source.Pause();
                    }

                    _cachedSFXState.Clear();
                    break;
                case MMSoundManagerTracks.Music:
                    foreach (var sound in _cachedMusicState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
                    ...
                    _cachedMusicState.Clear();
                    break;
                case MMSoundManagerTracks.UI:
                    foreach (var sound in _cachedUIState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
                    ...
                    _cachedUIState.Clear();
                    break;
                case MMSoundManagerTracks.Master:
                    foreach (var sound in _cachedMasterState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
                    ...
                    _cachedMasterState.Clear();
                    break;
                case MMSoundManagerTracks.Voices:
                    foreach (var sound in _cachedVoicesState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
                    ...
                    _cachedVoicesState.Clear();
                    break;
                case MMSoundManagerTracks.Other:
                    foreach (var sound in _cachedOtherState)
                    {
                        if (sound.Value)
                            sound.Key.Source.UnPause();
                        else
		...
		public virtual void OnMMEvent(MMSoundManagerTrackEvent soundManagerTrackEvent)
		{
			switch (soundManagerTrackEvent.TrackEventType)
			{
				...
				case MMSoundManagerTrackEventTypes.PauseTrack:
					...
					break;
                case MMSoundManagerTrackEventTypes.UnPauseTrack:
                    UnPauseTrack(soundManagerTrackEvent.Track);
                    break;
