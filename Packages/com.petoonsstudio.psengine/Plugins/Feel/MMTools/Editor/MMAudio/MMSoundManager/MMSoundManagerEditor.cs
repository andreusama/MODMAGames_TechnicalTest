...
namespace MoreMountains.Tools
{
	...
	public class MMSoundManagerEditor : Editor
	{
		...
		private static float _masterVolume, _musicVolume, _sfxVolume, _uiVolume, _voicesVolume;
        ...
        protected Color _voicesColorBase = MMColors.Bisque;
        protected Color _voicesColorMute;
        protected Color _voicesColorUnmute;
        protected Color _voicesColorPause;
        protected Color _voicesColorStop;
        protected Color _voicesColorPlay;
        protected Color _voicesColorFree;
		...
		protected virtual void OnEnable()
		{
            ...
            _voicesColorMute = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 1f);
            _voicesColorUnmute = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 0.9f);
            _voicesColorPause = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 0.8f);
            _voicesColorStop = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 0.7f);
            _voicesColorPlay = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 0.5f);
            _voicesColorFree = MMColors.MMColorize(_baseColor, _uiColorBase, _coloringMode, 0.4f);
        }
		...
		public override void OnInspectorGUI()
		{
			...
			if (_settingsSO != null)
			{
                ...
                _voicesVolume = _settingsSO.GetTrackVolume(MMSoundManager.MMSoundManagerTracks.Voices);
			...
			if ( ((_settingsSO != null) && _mmSoundManager.gameObject.activeInHierarchy))
			{
                ...
                DrawTrack("Voices Track", _mmSoundManager.settingsSo.Settings.VoicesOn, MMSoundManager.MMSoundManagerTracks.Voices, _voicesColorMute, _voicesColorUnmute, _voicesColorPause, _voicesColorStop, _voicesColorPlay, _voicesColorFree);
                DrawSaveLoadButtons();
			}
		}
		...
		protected virtual void DrawTrack(string title, bool mute, MMSoundManager.MMSoundManagerTracks track, Color muteColor, Color unmuteColor, Color pauseColor, Color stopColor, Color playColor, Color freeColor)
		{
			...
			switch (track)
			{
				...
				case MMSoundManager.MMSoundManagerTracks.UI:
					...
					break;
                case MMSoundManager.MMSoundManagerTracks.Voices:
                    newVolume = EditorGUILayout.Slider(_voicesVolume, MMSoundManagerSettings._minimalVolume, MMSoundManagerSettings._maxVolume);
                    if (newVolume != _voicesVolume) { _mmSoundManager.settingsSo.SetTrackVolume(MMSoundManager.MMSoundManagerTracks.Voices, newVolume); }
                    break;
            }
