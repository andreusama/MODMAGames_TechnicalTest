...
namespace sc.modeling.splines.editor
{
    ...
    public class SplineMesherInspector : Editor
    {
        ...
        private SerializedProperty sourceMeshes;
        private SerializedProperty firstSourceMesh;
        private SerializedProperty lastSourceMesh;
        private bool sourceFoldOut;
        ...
        private SerializedProperty collisionMeshes;
        private SerializedProperty firstCollisionMesh;
        private SerializedProperty lastCollisionMesh;
        private SerializedProperty useSameMesh;
        ...
        private SerializedProperty randomSequence;
        ...
        private void OnEnable()
        {
            ...
            sourceMeshes = serializedObject.FindProperty("sourceMeshes");
            firstSourceMesh = serializedObject.FindProperty("firstSourceMesh");
            lastSourceMesh = serializedObject.FindProperty("lastSourceMesh");
            ...
            SerializedProperty settings = serializedObject.FindProperty("settings");
            {
                ...
                collisionMeshes = settingsCollision.FindPropertyRelative("collisionMeshes");
                firstCollisionMesh = settingsCollision.FindPropertyRelative("firstCollisionMesh");
                lastCollisionMesh = settingsCollision.FindPropertyRelative("lastCollisionMesh");
                useSameMesh = settingsCollision.FindPropertyRelative("useSameMesh");
                ...
                randomSequence = settingsDistribution.FindPropertyRelative("randomSequence");
        ...
        private void DrawHeaderSection()
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(new GUIContent(EditorGUIUtility.IconContent(iconPrefix + "Settings").image, "Utility functions"), EditorStyles.miniButtonMid, GUILayout.Width(30f)))
                {
                    ...
                    {
                        if (sourceMeshes.objectReferenceValue)
                        {
                            SplineMeshEditor.RebuildAllInstancesUsingMesh(sourceMeshes.GetArrayElementAtIndex(0).objectReferenceValue as Mesh);
                        }
                    });
            ...
            //EditorGUILayout.BeginFadeGroup(setupSection.anim.faded);
            {
                ...
                //if (setupSection.Expanded)
                {
                    ...
                    if (splineContainer.objectReferenceValue)
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            sourceFoldOut = EditorGUILayout.Foldout(sourceFoldOut, "Source Meshes");

                            if (sourceFoldOut)
                            {
                                if (GUILayout.Button("+", GUILayout.MaxWidth(25)))
                                    sourceMeshes.arraySize++;

                                if (GUILayout.Button("-", GUILayout.MaxWidth(25)) && sourceMeshes.arraySize > 0)
                                    sourceMeshes.arraySize--;
                        ...
                        if (sourceFoldOut)
                        {
                            GUI.SetNextControlName($"firstSourceMesh");
                            EditorGUILayout.PropertyField(firstSourceMesh);

                            if (sourceMeshes.arraySize > 0)
                            {
                                for (int i = 0; i < sourceMeshes.arraySize; i++)
                                {
                                    GUI.SetNextControlName($"sourceMeshes_{i}");
                                    EditorGUILayout.PropertyField(sourceMeshes.GetArrayElementAtIndex(i));

                                    if (GUI.GetNameOfFocusedControl() == $"sourceMeshes_{i}")
                                        selectedMeshIndex = i;
                                }
                            }

                            GUI.SetNextControlName($"lastSourceMesh");
                            EditorGUILayout.PropertyField(lastSourceMesh);
                        }
                        ...
                        else
                        {
                            if (PreviewMesh)
                            {
                                ...
                                if (selectedMeshIndex >= 0)
                                    mesh = (Mesh)sourceMeshes.GetArrayElementAtIndex(selectedMeshIndex).objectReferenceValue;
                                else if (GUI.GetNameOfFocusedControl() == "firstSourceMesh")
                                    mesh = (Mesh)firstSourceMesh.objectReferenceValue;
                                else if (GUI.GetNameOfFocusedControl() == "lastSourceMesh")
                                    mesh = (Mesh)lastSourceMesh.objectReferenceValue;
                ...
                if (colliderSection.Expanded)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    {
                        ...
                        if (enableCollider.boolValue)
                        {
                            ...
                            EditorGUILayout.PropertyField(useSameMesh);

                            if (useSameMesh.boolValue)
                            {
                                colliderType.intValue = (int)Settings.ColliderType.Mesh;
                            }
                            else
                            {
                                ...
                                if (colliderType.intValue == (int)Settings.ColliderType.Mesh)
                                {
                                    using (new EditorGUILayout.HorizontalScope())
                                    {
                                        EditorGUILayout.PropertyField(firstCollisionMesh);
                                        ...
                                        if (GUILayout.Button(new GUIContent("Same", "Use the same mesh for collision as the source mesh"), EditorStyles.miniButton, GUILayout.Width(50f)))
                                            firstCollisionMesh.objectReferenceValue = firstSourceMesh.objectReferenceValue;
                                    }

                                    using (new EditorGUILayout.HorizontalScope())
                                    {
                                        EditorGUILayout.PropertyField(lastCollisionMesh);

                                        if (GUILayout.Button(new GUIContent("Same", "Use the same mesh for collision as the source mesh"), EditorStyles.miniButton, GUILayout.Width(50f)))
                                            lastCollisionMesh.objectReferenceValue = lastSourceMesh.objectReferenceValue;
                                    }

                                    EditorGUILayout.PropertyField(collisionMeshes);
                                }
        ...
        private void DrawDistribution()
        {
            ...
            EditorGUILayout.BeginFadeGroup(distributionSection.anim.faded);
            {
                if (distributionSection.Expanded)
                {
                    ...
                    EditorGUILayout.PropertyField(randomSequence);
