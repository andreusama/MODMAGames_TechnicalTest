    ...
    public class GdkUtilities
    {
        ...
        public static string RootPluginPath
        {
            get
            {
                ...
                {
#if UNITY_EDITOR
                    var guid = AssetDatabase.FindAssets($"t:Script {nameof(GdkUtilities)}");
                    var pathToThis = AssetDatabase.GUIDToAssetPath(guid[0]);
                    _rootPluginPath = Directory.GetParent(pathToThis).Parent.Parent.Parent.FullName;
#else
                    var path = Application.dataPath.Replace("Assets", "Packages/com.petoonsstudio.multiplatformutilities/Plugins/WindowsStore");
                    _rootPluginPath = path.Replace("/", @"\");
#endif
                }
        ...
        public static string GameConfigPath
        {
            get
            {
                if (!File.Exists(_gameConfigPath))
                {
                    ...
                    try
                    {
                        //Look for it in the ProjectSettings folder
                        string path = Application.dataPath.Replace("/", @"\");
                        path = path.Replace("Assets", "ProjectSettings");
